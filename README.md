...
이 프로젝트는 Spring Boot와 MySQL을 사용하여 개발된 일정 관리 API으로, 기존의 일정 관리 API(Scheduler-API)를 보완하여 만들 었습니다. 사용자는 일정을 생성, 조회, 수정, 삭제할 수 있으며, 필터링과 정렬 기능을 활용하여 원하는 데이터를 조회할 수 있습니다. 추가로 일정뿐 아니라 사용자 데이터도 관리할 수 있습니다. 필터링 값은 인덱스 처리하여 빠르게 필터링 가능합니다.
